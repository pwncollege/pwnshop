...
def auto_import_packages(
    base_module, base_path, ignore_dirs=(), ignore_files=(), scan_modules=True
):
    for lib_module_name in os.listdir(base_path):
        ...
        try:
            ...
            l.warning(
                "Unable to autoimport package %s.%s",
                base_module,
                lib_module_name,
                exc_info=True,
            )
            ...
            if scan_modules:
                for name, mod in auto_import_modules(
                    "%s.%s" % (base_module, lib_module_name),
                    lib_path,
...
def auto_import_modules(base_module, base_path, ignore_files=()):
    for proc_file_name in os.listdir(base_path):
        ...
        try:
            ...
            l.warning(
                "Unable to autoimport module %s.%s",
                base_module,
                proc_module_name,
                exc_info=True,
            )
